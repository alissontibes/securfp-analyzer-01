...
export const analyzeRFPRequirement = async (
  ...
  if (!apiKey) {
    throw new Error("API Key não configurada. Por favor, verifique suas configurações.");
  }
  ...
  // const targetDomain = context?.domain || "Segurança da Informação";
  ...
  while (attempts <= maxRetries) {
    try {
      ...
      const textResponse = response.text || "Sem resposta gerada.";
      ...
      if (explicitLink) {
          ...
          // Fallback to grounding chunks
          const chunks = response.candidates?.[0]?.groundingMetadata?.groundingChunks;
          if (chunks) {
              // Filter chunks to find technical ones first
              const technicalChunks = chunks.filter(c => {
                  const uri = c.web?.uri?.toLowerCase() || "";
                  return uri.includes('docs') || uri.includes('support') || uri.includes('kb') || uri.includes('sc1') || uri.includes('pdf');
              });
              
              const chunksToUse = technicalChunks.length > 0 ? technicalChunks : chunks;
      ...
      if (isRateLimit && attempts < maxRetries) {
          attempts++;
          const waitTime = 2500 * Math.pow(2, attempts); // Exponential backoff: 5s, 10s, 20s...
          console.warn(`Cota excedida (429). Retentativa ${attempts}/${maxRetries} em ${waitTime}ms...`);
          await delay(waitTime);
          continue;
      }
      
      // If not rate limit or max retries exceeded, throw
      break;
    }
  }

  console.error("Erro fatal na análise AI após retentativas:", lastError);
  throw new Error(lastError?.message || "Falha ao conectar com o serviço de IA. Verifique sua cota ou chave de API.");
};